...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['tambah_indikator'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO rkb (id_pegawai, id_rhk, uraian_kegiatan, kuantitas, satuan, bulan, tahun) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("iissiii", $id_pegawai, $id_rhk, $uraian_kegiatan, $kuantitas, $satuan, $bulan, $tahun);
        
        if ($stmt->execute()) {
            $message = "Indikator RKB berhasil ditambahkan.";
    ...
    if (isset($_POST['edit_indikator'])) {
        ...
        $stmt = $conn->prepare("UPDATE rkb SET id_rhk=?, uraian_kegiatan=?, kuantitas=?, satuan=? WHERE id_rkb=?");
        $stmt->bind_param("isssi", $id_rhk, $uraian_kegiatan, $kuantitas, $satuan, $id_rkb);
        
        if ($stmt->execute()) {
            $message = "Indikator RKB berhasil diperbarui.";
            $message_type = "success";
        } else {
            $message = "Error: " . $stmt->error;
            $message_type = "danger";
    ...
    if (isset($_POST['hapus_indikator'])) {
        ...
        $stmt = $conn->prepare("DELETE FROM rkb WHERE id_rkb=?");
        $stmt->bind_param("i", $id_rkb);
        
        if ($stmt->execute()) {
            $message = "Indikator RKB berhasil dihapus.";
            $message_type = "success";
        } else {
            $message = "Error: " . $stmt->error;
            $message_type = "danger";
    ...
    if (isset($_POST['update_periode'])) {
        ...
        $stmt = $conn->prepare("UPDATE rkb SET tahun=?, bulan=? WHERE id_rkb=?");
        $stmt->bind_param("iii", $tahun, $bulan, $id_rkb);
        
        if ($stmt->execute()) {
            $message = "Periode RKB berhasil diperbarui.";
            $message_type = "success";
        } else {
            $message = "Error: " . $stmt->error;
            $message_type = "danger";
    ...
    if (isset($_POST['update_status'])) {
        ...
        $stmt = $conn->prepare("UPDATE rkb SET status_verval=? WHERE id_rkb=?");
        $stmt->bind_param("si", $status_verval, $id_rkb);
        
        if ($stmt->execute()) {
            $message = "Status RKB berhasil diperbarui.";
            $message_type = "success";
        } else {
            $message = "Error: " . $stmt->error;
            $message_type = "danger";
    ...
    if (isset($_POST['update_status_periode'])) {
        ...
        if ($status_verval === 'draft') {
            $stmt = $conn->prepare("UPDATE rkb SET status_verval=NULL WHERE id_pegawai=? AND tahun=? AND bulan=?");
            $stmt->bind_param("iii", $id_pegawai, $tahun, $bulan);
        } else {
            // Update status untuk semua RKB pada periode tertentu
            $stmt = $conn->prepare("UPDATE rkb SET status_verval=? WHERE id_pegawai=? AND tahun=? AND bulan=?");
            $stmt->bind_param("siii", $status_verval, $id_pegawai, $tahun, $bulan);
        }
        
        if ($stmt->execute()) {
            $message = "Status RKB periode " . $bulan . "/" . $tahun . " berhasil diperbarui.";
            $message_type = "success";
        } else {
            $message = "Error: " . $stmt->error;
            $message_type = "danger";
...
// Get data RKB berdasarkan pegawai yang dipilih
$rkb_result = null;
...
if ($selected_id_pegawai) {
    ...
    if ($selected_pegawai) {
        ...
        // Get data RHK untuk pegawai ini (untuk dropdown)
        $rhk_query = $conn->prepare("SELECT id_rhk, nama_rhk FROM rhk WHERE id_pegawai = ? AND tahun = ? AND bulan = ? ORDER BY nama_rhk");
        $rhk_query->bind_param("iii", $selected_id_pegawai, $filter_tahun, $filter_bulan);
        $rhk_query->execute();
        $rhk_options = $rhk_query->get_result();
        $rhk_query->close();
        
        // Get data RKB untuk pegawai yang dipilih dengan filter periode
        $stmt = $conn->prepare("
            SELECT r.*, p.nama, p.jabatan, p.unit_kerja, rh.nama_rhk 
            FROM rkb r 
            JOIN pegawai p ON r.id_pegawai = p.id_pegawai 
            LEFT JOIN rhk rh ON r.id_rhk = rh.id_rhk
            WHERE r.id_pegawai = ? AND r.tahun = ? AND r.bulan = ?
            ORDER BY r.created_at DESC
        ");
        $stmt->bind_param("iii", $selected_id_pegawai, $filter_tahun, $filter_bulan);
        $stmt->execute();
        $rkb_result = $stmt->get_result();
        $stmt->close();
    }
}

// Get status RKB untuk periode ini
$status_query = $conn->prepare("SELECT DISTINCT status_verval FROM rkb WHERE id_pegawai = ? AND tahun = ? AND bulan = ? LIMIT 1");
...
<div id="layoutSidenav_content">
    <main>
        <div class="container-fluid px-4">
            <h1 class="mt-4 mb-3"><i class="fas fa-calendar-week"></i> Manajemen RKB Pegawai</h1>
            <p class="lead">Kelola indikator Rencana Kegiatan Bulanan untuk setiap pegawai.</p>
            ...
            <?php if (isset($_GET['debug'])): ?>
                <div class="alert alert-info">
                    ...
                    Total Pegawai: <?php echo $pegawai_result->num_rows; ?><br>
                    Selected ID: <?php echo $selected_id_pegawai; ?><br>
                ...
                <div class="card-header bg-gradient-info text-white border-0">
                    <div class="row align-items-center">
                        <div class="col">
                            ...
                            <small class="opacity-75">Cari dan pilih pegawai untuk mengelola RKB</small>
                        </div>
                        <div class="col-auto">
                        ...
                        <div class="row g-3 align-items-end">
                            ...
                            <div class="col-md-3">
                                <button type="submit" 
                                    ...
                                    <span class="fw-bold">Lihat RKB</span>
                                </button>
                            </div>
                        </div>
                    </form>
                ...
                <div class="card shadow-sm">
                    <div class="card-header bg-primary text-white">
                        <div class="row align-items-center">
                            <div class="col">
                                <h5 class="mb-0">
                                    <i class="fas fa-calendar-week"></i> Daftar RKB - <?php echo htmlspecialchars($selected_pegawai['nama']); ?>
                            ...
                            <div class="col-auto">
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <?php if ($rkb_result && $rkb_result->num_rows > 0): ?>
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    <?php 
                                    $no = 1;
                                    if ($rkb_result && $rkb_result->num_rows > 0):
                                        while ($row = $rkb_result->fetch_assoc()):
                                    ?>
                                        <tr>
                                            <td><?php echo $no++; ?></td>
                                            <td>
                                                <small class="text-muted"><?php echo htmlspecialchars($row['nama_rhk'] ?? 'RHK tidak ditemukan'); ?></small>
                                            </td>
                                            <td><?php echo htmlspecialchars($row['uraian_kegiatan']); ?></td>
                                            <td class="text-center"><?php echo htmlspecialchars($row['kuantitas']); ?></td>
                                            <td><?php echo htmlspecialchars($row['satuan']); ?></td>
                                            <td><?php echo date('d/m/Y H:i', strtotime($row['created_at'])); ?></td>
                                            <td class="text-center">
                                                <!-- Action Buttons -->
                                                <div class="btn-group" role="group">
                                                    <button class="btn btn-sm btn-warning" data-bs-toggle="modal" data-bs-target="#modalEditIndikator_<?php echo $row['id_rkb']; ?>" title="Edit">
                                                        <i class="fas fa-edit"></i>
                                                    </button>
                                                    <button type="button" class="btn btn-sm btn-danger" onclick="hapusIndikator(<?php echo $row['id_rkb']; ?>)" title="Hapus">
                                        ...
                                        <div class="modal fade" id="modalEditIndikator_<?php echo $row['id_rkb']; ?>" tabindex="-1">
                                            <div class="modal-dialog modal-lg">
                                                <div class="modal-content">
                                                    <form method="POST">
                                                        <div class="modal-header bg-warning text-white">
                                                            <h5 class="modal-title">Edit RKB</h5>
                                                            <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                                        </div>
                                                        <div class="modal-body">
                                                            <input type="hidden" name="edit_indikator" value="1">
                                                            <input type="hidden" name="id_rkb" value="<?php echo $row['id_rkb']; ?>">
                                                            
                                                            <div class="mb-3">
                                                                <label class="form-label">RHK Terkait *</label>
                                                                <select name="id_rhk" class="form-select" required>
                                                                    <option value="">Pilih RHK</option>
                                                                    <?php 
                                                                    $rhk_options->data_seek(0);
                                                                    while ($rhk = $rhk_options->fetch_assoc()): 
                                                                    ?>
                                                                        <option value="<?php echo $rhk['id_rhk']; ?>" 
                                                                                <?php echo ($row['id_rhk'] == $rhk['id_rhk']) ? 'selected' : ''; ?>>
                                                            ...
                                                            <div class="mb-3">
                                                                ...
                                                                <textarea name="uraian_kegiatan" class="form-control" rows="3" required><?php echo htmlspecialchars($row['uraian_kegiatan']); ?></textarea>
                                                            </div>
                                                            ...
                                                            <div class="row">
                                                                <div class="col-md-6">
                                                                    <div class="mb-3">
                                                                        ...
                                                                        <input type="text" name="kuantitas" class="form-control" required value="<?php echo htmlspecialchars($row['kuantitas']); ?>">
                                                                    </div>
                                                                </div>
                                                                <div class="col-md-6">
                                                                    <div class="mb-3">
                                                                        <label class="form-label">Satuan *</label>
                                                                        <input type="text" name="satuan" class="form-control" required value="<?php echo htmlspecialchars($row['satuan']); ?>" placeholder="Contoh: dokumen, kegiatan, jam">
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                    ...
                                    ?>
                                        <tr>
                                            <td colspan="7" class="text-center">
                                                Belum ada data RKB untuk pegawai ini pada periode <?php echo $bulan_names[$filter_bulan] . ' ' . $filter_tahun; ?>
                                            </td>
                                        </tr>
                                    <?php endif; ?>
                                </tbody>
                        ...
                        <div class="modal-content">
                            <form method="POST">
                                <div class="modal-header bg-success text-white">
                                    <h5 class="modal-title">Tambah RKB untuk <?php echo htmlspecialchars($selected_pegawai['nama']); ?></h5>
                                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                                </div>
                                <div class="modal-body">
                                    ...
                                    <div class="alert alert-info">
                                        ...
                                        RKB ini akan dibuat untuk periode: <strong><?php echo $bulan_names[$filter_bulan] . ' ' . $filter_tahun; ?></strong>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="form-label">RHK Terkait *</label>
                                        <select name="id_rhk" class="form-select" required>
                                            <option value="">Pilih RHK yang akan dijabarkan</option>
                                            <?php 
                                            if ($rhk_options->num_rows > 0) {
                                                $rhk_options->data_seek(0);
                                                while ($rhk = $rhk_options->fetch_assoc()): 
                                            ?>
                                                <option value="<?php echo $rhk['id_rhk']; ?>">
                                            ...
                                            <?php 
                                                ...
                                                echo '<option value="">Tidak ada RHK untuk periode ini</option>';
                                            }
                                            ?>
                                        </select>
                                        <small class="text-muted">Pilih RHK yang akan dijabarkan menjadi kegiatan bulanan</small>
                                    </div>
                                    
                                    <div class="mb-3">
                                        <label class="form-label">Uraian Kegiatan *</label>
                                        <textarea name="uraian_kegiatan" class="form-control" rows="3" required 
                                                  placeholder="Contoh: Melaksanakan pembelajaran di kelas VII A mata pelajaran Matematika"></textarea>
                                        <small class="text-muted">Jelaskan kegiatan spesifik yang akan dilakukan</small>
                                    </div>
                                    ...
                                    <div class="row">
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                <label class="form-label">Kuantitas *</label>
                                                <input type="text" name="kuantitas" class="form-control" required 
                                                       placeholder="Contoh: 24">
                                                <small class="text-muted">Jumlah/volume kegiatan</small>
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="satuan" class="form-control" required 
                                                       placeholder="Contoh: jam pelajaran">
                                                <small class="text-muted">Satuan pengukuran (jam, dokumen, kegiatan, dll)</small>
                                            </div>
                                        </div>
                                    </div>
                                </div>
            ...
            <?php else: ?>
                ...
                <div class="card shadow-sm">
                    <div class="card-body text-center py-5">
                        ...
                        <h5 class="text-muted">Silakan pilih pegawai untuk melihat dan mengelola data RKB</h5>
                        <p class="text-muted">Gunakan filter di atas untuk memilih pegawai yang akan dikelola data RKB-nya.</p>
                    </div>
                </div>
            <?php endif; ?>
        </div>
...
.info-item:hover { transform: translateY(-5px); box-shadow: 0 5px 20px rgba(0,0,0,0.15); }
.info-icon { margin-right: 15px; font-size: 1.5rem; }
.periode-section { animation: fadeInScale 0.8s ease 0.3s both; }
...
.search-result-item.active { background-color: #e3f2fd; border-left: 4px solid #007bff; }
...
function showLoadingAlert(title, text) {
    Swal.fire({
        title: title, text: text, allowOutsideClick: false, allowEscapeKey: false,
        showConfirmButton: false, background: '#fff',
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (tahunSelect) {
        tahunSelect.addEventListener('change', function() {
            ...
            Swal.fire({
                title: '📅 Mengubah Periode', text: `Memfilter RKB untuk periode ${bulanNama} ${tahunNama}`,
                icon: 'info', timer: 1500, showConfirmButton: false
            }).then(() => { this.closest('form').submit(); });
        });
    }
        ...
        Swal.fire({
            ...
            html: `
                <div class="text-center">
                    ...
                    <div class="badge bg-primary">Periode: <?php echo $bulan_names[$filter_bulan] . ' ' . $filter_tahun; ?></div>
                </div>
            `,
...
if (selectPegawai) {
    selectPegawai.addEventListener('change', function() {
        if (this.value) {
            ...
            showLoadingAlert('🔍 Memuat Data RKB', `Sedang mengambil data RKB untuk ${nama}...`);
...
function confirmTambah() { return true; }
function confirmEdit() { return true; }

// Function untuk hapus indikator RKB
function hapusIndikator(idRkb) {
    Swal.fire({
        title: '🗑️ Hapus Indikator RKB?',
        html: `<div class="text-center"><i class="fas fa-exclamation-triangle fa-3x text-warning mb-3"></i><p>Data yang dihapus <strong>tidak dapat dikembalikan!</strong></p></div>`,
        icon: 'warning', showCancelButton: true, confirmButtonColor: '#dc3545',
        confirmButtonText: '🗑️ Ya, Hapus!', cancelButtonText: '❌ Batal'
    }).then((result) => {
        if (result.isConfirmed) {
            showLoadingAlert('🗑️ Menghapus Data', 'Sedang menghapus indikator RKB...');
            setTimeout(() => {
                const form = document.createElement('form');
                form.method = 'POST'; form.style.display = 'none';
                
                const inputAction = document.createElement('input');
                inputAction.type = 'hidden'; inputAction.name = 'hapus_indikator'; inputAction.value = '1';
                
                const inputId = document.createElement('input');
                inputId.type = 'hidden'; inputId.name = 'id_rkb'; inputId.value = idRkb;
...
// Function untuk update periode RKB
function updatePeriode(idRkb, tahun, bulan) {
    Swal.fire({
        title: '📅 Set Periode RKB',
        html: `<div class="text-center"><i class="fas fa-calendar-plus fa-3x text-info mb-3"></i><p>Atur periode RKB ini ke:</p><div class="badge bg-primary fs-6">${tahun} - Bulan ${bulan}</div></div>`,
        icon: 'question', showCancelButton: true, confirmButtonColor: '#17a2b8',
        confirmButtonText: '📅 Ya, Set Periode!', cancelButtonText: '❌ Batal'
    }).then((result) => {
        if (result.isConfirmed) {
            showLoadingAlert('📅 Mengatur Periode', 'Sedang mengupdate periode RKB...');
            setTimeout(() => {
                const form = document.createElement('form');
                form.method = 'POST'; form.style.display = 'none';
                
                [['update_periode', '1'], ['id_rkb', idRkb], ['tahun', tahun], ['bulan', bulan]].forEach(([name, value]) => {
...
// Function untuk update status RKB
function updateStatus(idRkb, status) {
    const statusText = status === 'verified' ? 'Verified' : (status === 'rejected' ? 'Rejected' : 'Pending');
    const statusIcon = status === 'verified' ? '✅' : (status === 'rejected' ? '❌' : '⏳');
    
    Swal.fire({
        title: `${statusIcon} Update Status RKB`,
        html: `<div class="text-center"><p>Ubah status RKB menjadi:</p><div class="badge bg-${status === 'verified' ? 'success' : (status === 'rejected' ? 'danger' : 'warning')} fs-6">${statusText}</div></div>`,
        icon: 'question', showCancelButton: true, confirmButtonColor: '#17a2b8',
        confirmButtonText: `${statusIcon} Ya, Update!`, cancelButtonText: '❌ Batal'
    }).then((result) => {
        if (result.isConfirmed) {
            showLoadingAlert('🔄 Mengupdate Status', 'Sedang mengupdate status RKB...');
            setTimeout(() => {
                const form = document.createElement('form');
                form.method = 'POST'; form.style.display = 'none';
                
                [['update_status', '1'], ['id_rkb', idRkb], ['status_verval', status]].forEach(([name, value]) => {
                    const input = document.createElement('input');
                    input.type = 'hidden'; input.name = name; input.value = value;
                    form.appendChild(input);
                });
...
// Function untuk update status periode RKB
function updateStatusPeriode(status) {
    ...
    Swal.fire({
        title: `${statusIcon[status]} Update Status RKB`,
        html: `<div class="text-center">
            <p>Ubah status RKB periode ini menjadi:</p>
            <div class="badge bg-${statusColor[status]} fs-6">${statusText[status]}</div>
            <br><br>
            <small class="text-muted">Status akan diterapkan untuk semua RKB pada periode <?php echo $bulan_names[$filter_bulan] . ' ' . $filter_tahun; ?></small>
        </div>`,
        icon: 'question', 
        showCancelButton: true, 
        confirmButtonColor: '#17a2b8',
        confirmButtonText: `${statusIcon[status]} Ya, Update!`, cancelButtonText: '❌ Batal'
    }).then((result) => {
        if (result.isConfirmed) {
            showLoadingAlert('🔄 Mengupdate Status', 'Sedang mengupdate status RKB periode...');
            setTimeout(() => {
                const form = document.createElement('form');
                form.method = 'POST'; 
                ...
                [
                    ['update_status_periode', '1'], 
                    ['id_pegawai', <?php echo $selected_id_pegawai; ?>], 
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const btnLihatRKB = document.getElementById('btnLihatRKB');
    
    // Check if elements exist
    if (!searchInput || !searchResults || !selectPegawai || !selectedPegawaiDiv || !btnLihatRKB) {
    ...
    function displaySearchResults(results, query) {
        ...
        if (results.length === 0) {
            html = '<div class="no-results p-4 text-center text-muted"><i class="fas fa-search mb-2"></i><br>Tidak ada pegawai yang ditemukan untuk: "' + query + '"</div>';
        } else {
    ...
    function selectPegawaiFromSearch(id) {
        ...
        if (option) {
            ...
            searchResults.classList.add('d-none');
            selectedPegawaiDiv.classList.remove('d-none');
            btnLihatRKB.disabled = false;
            
            Swal.fire({
                title: '✅ Pegawai Dipilih!',
                html: `<div class="text-center"><i class="fas fa-user-check fa-3x text-success mb-3"></i><h5>${pegawaiData.nama}</h5><p class="text-muted">${pegawaiData.jabatan}</p></div>`,
